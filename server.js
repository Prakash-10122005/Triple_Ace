...
// ─────────────────────────────────────────
//  SOCKET.IO — polling first (works on Render/Railway)
// ─────────────────────────────────────────
...
// ─────────────────────────────────────────
//  STATIC FILES — served from root folder
//  /socket.io/* is handled by Socket.io automatically BEFORE this
// ─────────────────────────────────────────
...
// ─────────────────────────────────────────
//  ROOMS — defined BEFORE any endpoint uses it
// ─────────────────────────────────────────
const rooms = new Map();

// Health check endpoint
app.get('/health', (req, res) => {
  res.json({ status:'ok', uptime:process.uptime(), rooms:rooms.size, ts:new Date().toISOString() });
});
...
function createRoom(hostSocketId) {
  ...
  const room = {
    code, hostId:hostSocketId, phase:'lobby',
    players:[], drawPile:[], roundCards:[],
    leadSuit:null, leadPid:null, currentPlayer:0,
    roundNum:1, totalDead:0, finishedCount:0,
    roundEnded:false, log:[],
...
function roomBroadcast(room, event, data) { io.to(room.code).emit(event, data); }
...
function sendState(room) { roomBroadcast(room, 'state', getPublicState(room)); }

function getPublicState(room) {
  return {
    code:room.code, phase:room.phase,
    players: room.players.map((p,i)=>({
      index:i, name:p.name, handCount:p.hand.length,
      eliminated:p.eliminated, finishRank:p.finishRank, revealVal:p.revealVal
    })),
    drawPileCount:room.drawPile.length,
    roundCards:room.roundCards,
    leadSuit:room.leadSuit, leadPid:room.leadPid,
    currentPlayer:room.currentPlayer, roundNum:room.roundNum,
    totalDead:room.totalDead, roundEnded:room.roundEnded,
...
function startGame(room) {
  ...
  room.drawPile=deck; room.roundCards=[];
  room.leadSuit=null; room.leadPid=null;
  room.roundNum=1; room.totalDead=0;
  room.finishedCount=0; room.roundEnded=false; room.log=[];

  for (const p of room.players) {
    p.hand=[]; p.eliminated=false; p.finishRank=null; p.revealVal=null;
  }
...
function playVettu(room, pid, suit) {
  ...
  const player=room.players[pid];
  const hasSuit=player.hand.some(c=>c.s===room.leadSuit);
  if (hasSuit) return {err:`Must follow ${SNAME[room.leadSuit]}`};
...
function resolveRound(room) {
  ...
  if (hadVettu){
    ...
    addLog(room,`🔴 VETTU! ALL ${allCards.length} card(s) → ${highestPlayer.name}'s hand.`,'le-v');
  } else {
  ...
  // Check finishers (game ends when only 1 player left = LOSER)
  for (const p of room.players){
    ...
    for (const [code,room] of rooms.entries()){
      ...
      if (p){
        ...
        if (room.players.every(pl=>!io.sockets.sockets.get(pl.socketId))){
          ...
          console.log(`Room ${code} grace period started.`);
        }
        break;
      }
    }
...
// ─────────────────────────────────────────
//  ROOM EXPIRY — 5 min grace after empty
// ─────────────────────────────────────────
setInterval(()=>{
  const now=Date.now();
  for (const [code,room] of rooms.entries()){
